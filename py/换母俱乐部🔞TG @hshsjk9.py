...
_proxy_headers = {
    ...
    'Referer': 'https://19hs1q07w8.minba-abus.buzz/',
}
...
class Spider(BaseSpider):
    ...
    HOSTS = ['https://19hs1q07w8.minba-abus.buzz']
    DEFAULT_CATEGORIES = [
    ...
    def _parse_list(self, html):
        ...
        # 模式 A：.vods > .vod 封面卡片
        cards = re.findall(
            r'<div\s+class=["\']?[^"\'>]*\bvod\b[^"\'>]*["\']?\s*>'
            r'\s*<div\s+class=["\']?[^"\'>]*\bvod-img\b[^"\'>]*["\']?\s*>(.*?)</div>\s*'
            r'<div\s+class=["\']?[^"\'>]*\bvod-txt\b[^"\'>]*["\']?\s*>(.*?)</div>\s*'
            r'</div>',
            html, re.S
        )
        ...
        if cards:
            for img_block, txt_block in cards:
                ...
                title = vid
                t_match = re.search(r'<a[^>]*>(.*?)</a>', txt_block, re.S)
                if t_match:
                    title = re.sub(r'<[^>]+>', '', t_match.group(1)).strip() or vid
                ...
                for pic_pat in (
                    ...
                    pm = re.search(pic_pat, img_block, re.S)
                    if pm:
                        pic = pm.group(1).strip()
                        if 'loading.gif' in pic or 'error' in pic:
                            pic = ''
                            continue
                        break
                ...
                remarks = ''
                rem_match = re.search(
                    r'<span[^>]*class=["\'][^"\']*\bpic-text\b[^"\']*["\'][^>]*>(.*?)</span>',
                    img_block, re.S
                )
                if not rem_match:
                    rem_match = re.search(
                        r'<span[^>]*class=["\'][^"\']*\bvideo-grade\b[^"\']*["\'][^>]*>(.*?)</span>',
                        img_block, re.S
                    )
                if rem_match:
                    remarks = re.sub(r'<[^>]+>', '', rem_match.group(1)).strip()

                items.append({
                    'vod_id': vid,
                    'vod_name': title,
                    'vod_pic': self._proxy_url(self._abs_url(pic)),
                    'vod_remarks': remarks,
                })

        # 模式 B：.pic 图文列表
        if not items:
            pic_blocks = re.findall(
                r'<div\s+class=["\']?[^"\'>]*\bpic\b[^"\'>]*["\']?\s*>\s*<ul>(.*?)</ul>',
                html, re.S
            )
            for block in pic_blocks:
                links = re.findall(
                    r'<a[^>]+href=["\'](/vodplay/(\d+)-\d+-\d+/?(?:\.html)?)["\'][^>]*>'
                    r'.*?<img[^>]+(?:data-original|data-src|src)=["\']([^"\']+)["\'][^>]*>.*?</a>',
                    block, re.S
                )
                for href, vid, pic in links:
                    if vid in seen_vids:
                        continue
                    seen_vids.add(vid)
                    t_match = re.search(
                        r'<a[^>]+href=["\']' + re.escape(href) + r'["\'][^>]*(?:title=["\']([^"\']+)["\'])?[^>]*>'
                        r'(.*?)</a>', block, re.S
                    )
                    title = vid
                    if t_match:
                        title = t_match.group(1).strip() if t_match.group(1) else re.sub(r'<[^>]+>', '', t_match.group(2)).strip()
                        title = title or vid
                    items.append({
                        'vod_id': vid,
                        'vod_name': title,
        ...
        # 模式 C：.pic2 纯文字列表
        if not items:
            pic2_blocks = re.findall(
                r'<div\s+class=["\']?[^"\'>]*\bpic2\b[^"\'>]*["\']?\s*>\s*<ul>(.*?)</ul>',
                html, re.S
            )
            for block in pic2_blocks:
                links = re.findall(
                    r'<a[^>]+href=["\'](/vodplay/(\d+)-\d+-\d+/?(?:\.html)?)["\'][^>]*title=["\']([^"\']+)["\'][^>]*>.*?</a>',
                    block, re.S
                )
                if not links:
    ...
    def _get_list(self, tid, page):
        ...
        if page == 1:
            ...
            urls_to_try.append(f'{self.host}/vodtype/{tid}_{page}.html')
        for url in urls_to_try:
            html = self._fetch(url, referer=self.host + '/vod/')
            if html:
                return self._parse_list(html)
    ...
    def playerContent(self, flag, id, vipFlags=None):
        ...
        config_match = re.search(r'MacPlayerConfig\.player_list\s*=\s*(\{.*?\});', html, re.S)
        if config_match:
            try:
                config_json = json.loads(config_match.group(1))
                self._log(f'加载解析器配置: {list(config_json.keys())}')
            except:
                pass
